...
techniciansRouter.post(
  ...
  async (req, res) => {
    try {
      ...
      const jenisKeahlianArray = Array.isArray(jenisKeahlian)
      ? jenisKeahlian
      : jenisKeahlian.split(",").map((item) => item.trim());
      ...
      const technician = {
        ...
        email,
        noHandphone,
        jenisKeahlian: jenisKeahlianArray,
        NIK,
        linkSertifikasi,
        linkPortofolio,
        jenisKendaraan,
...
techniciansRouter.post("/:technicianId/feedback", async (req, res) => {
  try {
    ...
    // Ubah nilai rating menjadi tipe data float
    const floatRating = parseFloat(rating);

    // Validasi rating agar berada dalam rentang 1 hingga 5
    if (isNaN(floatRating) || floatRating < 1 || floatRating > 5) {
      return res
        .status(400)
        .json({ error: "Rating harus berupa angka antara 1 hingga 5" });
    }
